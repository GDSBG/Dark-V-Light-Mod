...
import net.boogaeye.darkvlight.block.BurnedGazeWoodBlock;
...
public class BurnedGazeForestBiome extends DarkVLightModElements.ModElement {
	@ObjectHolder("dark_v_light:burned_gaze_forest")
	public static final CustomBiome biome = null;
	public BurnedGazeForestBiome(DarkVLightModElements instance) {
		super(instance, 10);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.8f).depth(0.1f).scale(0.2f).temperature(0.7f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.NONE).waterColor(-10066432).waterFogColor(-10066432)
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(DarkendGrassBlock.block.getDefaultState(),
							DarkDirtBlock.block.getDefaultState(), DarkDirtBlock.block.getDefaultState())));
			setRegistryName("burned_gaze_forest");
			DefaultBiomeFeatures.addCarvers(this);
			DefaultBiomeFeatures.addStructures(this);
			DefaultBiomeFeatures.addMonsterRooms(this);
			DefaultBiomeFeatures.addOres(this);
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(BurnedGazeWoodBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(DarkStoneBlock.block.getDefaultState()))).baseHeight(8)
									.setSapling((net.minecraftforge.common.IPlantable) Blocks.JUNGLE_SAPLING).build())
					.withPlacement(Placement.COUNT_EXTRA_HEIGHTMAP.configure(new AtSurfaceWithExtraConfig(7, 0.1F, 1))));
			this.addSpawn(EntityClassification.AMBIENT, new Biome.SpawnListEntry(DarkendFloaterEntity.entity, 30, 2, 16));
		}
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == DarkendGrassBlock.block.getDefaultState().getBlock()
							|| ground == DarkDirtBlock.block.getDefaultState().getBlock())
							&& (ground2 == DarkendGrassBlock.block.getDefaultState().getBlock()
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == DarkStoneBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, DarkStoneBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, BurnedGazeWoodBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BurnedGazeWoodBlock.block.getDefaultState().getBlock()
					|| blockType == DarkStoneBlock.block.getDefaultState().getBlock()
					|| blockType == DarkendGrassBlock.block.getDefaultState().getBlock()
